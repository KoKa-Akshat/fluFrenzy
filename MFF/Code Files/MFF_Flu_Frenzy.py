...
Main File (only add code here that works in MFF_Flu_Frenzy.py)
======================================================================================"""
...
class CharacterSelection(tk.Frame):
    """characters for our game"""

    def __init__(self, master=None):
        """Initialize: character selection window and character buttons."""

        # the master parameter is used to connect our two classes together by giving it a reference to the same parent widget (tk.Frame), allowing the classes to interact with each other and share information as needed.
        self.master = master
        self.master.title("In-Game-Characters")
        self.master.geometry("500x500")

        self.canvas = tk.Canvas(self.master, width=500, height=500, relief=tk.RIDGE)
...
class Flu_Frenzy(tk.Frame):
    ...
    def __init__(self, master=None, val=None):
        ...
        """Setting up the display counters that need to be constantly updated"""
        ...
        """Canvas"""
        self.canvaScreen = tk.LabelFrame(self.window, text="Game Screen", padx=5, pady=5, bg="khaki", fg="black",
                                         relief=tk.RIDGE)
        ...
        """Notebook"""
        self.notebook_frame = tk.LabelFrame(self.window, text="Scenarios", padx=5, pady=5, bg="khaki", fg="black",
                                            relief=tk.RIDGE)
    ...
    def create_notebook(self):
        """Creates the notebook object with tabs and radiobuttons for out trivia"""
        ...
        # Three instances of the tk.IntVar class, used to store the value of the selected radio button in each of the three tabs of the Notebook widget.
        ...
        self.notebook.add(self.tab1, text="How many\n people you saw?", compound=tk.TOP)
        self.notebook.add(self.tab2, text="What floor\n you went?", compound=tk.TOP)
        ...
        self.create_radioButtons(self.tab1, "10-25 people", "25-40 people", "more than 40 people", 1, 2, 3,
                                 self.tab2_rb_var, self.tab2)
        self.create_radioButtons(self.tab2, "1", "2", "both 1 & 2", 1, 2, 3, self.tab3_rb_var, self.tab3)
        self.create_radioButtons(self.tab3, "1-5 bros", "5-10 bros", "more than 10 bros", 1, 2, 3, self.tab1_rb_var,
                                 self.tab1)
    ...
    def create_radioButtons(self, parent, a, b, c, a_val, b_val, c_val, var, next_tab):
        ...
        self.rb1 = tk.Radiobutton(parent, text=a, value=a_val, variable=var,
                                  command=lambda: self.delayed_tab_switch(next_tab))
        self.rb1.grid(row=1, column=1)
        self.rb2 = tk.Radiobutton(parent, text=b, value=b_val, variable=var,
                                  command=lambda: self.delayed_tab_switch(next_tab))
        self.rb2.grid(row=2, column=1)
        self.rb3 = tk.Radiobutton(parent, text=c, value=c_val, variable=var,
                                  command=lambda: self.delayed_tab_switch(next_tab))
    ...
    def create_widgets(self):
        ...
        self.display_box = tk.Button(self.notebook_frame, text="Get Value", command=self.get_selected_value, height=4,
                                     width=20, fg="blue", font="Arial 13 bold")
        ...
        self.Infected_value = ttk.Progressbar(self.notebook_frame, orient='horizontal', length=200,
                                              mode='indeterminate')
        ...
        self.button_1.grid(row=2, column=1, columnspan=1, pady=6, sticky=tk.W)
        ...
        self.button_2.grid(row=3, column=1, columnspan=1, sticky=tk.W)
        ...
        self.button_3.grid(row=2, column=2, columnspan=1, sticky=tk.W)
        ...
        self.button_5.grid(row=3, column=5, columnspan=1, sticky=tk.W)
        ...
        self.button_6.grid(row=2, column=5, columnspan=1, sticky=tk.W)
        self.button_6.bind("<ButtonPress-1>", self.gotoOM)
        self.button_7 = tk.Button(self.canvaScreen, text="7. DeWitt Wallace Library", padx=5, pady=2, bg="grey",
                                  fg="black")
        self.button_7.grid(row=2, column=4, columnspan=1, sticky=tk.W)
        ...
        self.button_8.grid(row=3, column=4, columnspan=1, sticky=tk.W)
        ...
        self.button_9.grid(row=3, column=3, columnspan=1, sticky=tk.W)
        ...
        self.button_10.grid(row=3, column=2, columnspan=1, sticky=tk.W)
        ...
        self.button_14.grid(row=2, column=3, columnspan=1, sticky=tk.W)
    ...
    def gotoLC(self, event):
        ...
        self.canvas.after(3000, lambda: tk.messagebox.showinfo("Information",
                                                               "Please answer the Trivia on your left before going to next building."))
    ...
    def calcRo(self):
        """Calculates the Ro value and the # of people infected from user input"""
        # Getting the input values
        label_text = str(self.display_box['text'])
        list = label_text.split("\n")
        # R0 = (transmission rate x duration of infectiousness x contact rate) / recovery rate
